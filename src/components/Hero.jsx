...
function SensorRow({ icon: Icon, label, value, detail, color }) {
  return (
    <div className="flex items-center justify-between rounded-xl border border-zinc-200 bg-zinc-50 p-3 dark:border-white/[0.07] dark:bg-white/[0.025]">
    ...
    <section id="home" ref={sectionRef} className="relative h-[150vh] bg-white transition-colors duration-300 dark:bg-[#09090b]">
      ...
      <div className="sticky top-[20vh] z-10 mx-auto max-w-6xl px-6">
        <motion.div style={{ opacity: textOpacity, scale: textScale }} className="mx-auto max-w-4xl text-center">
          ...
          <h1 className="text-5xl font-bold tracking-tight text-zinc-950 sm:text-6xl lg:text-7xl dark:text-zinc-100">
            {t.hero.title}
          </h1>
          <p className="mx-auto mt-6 max-w-3xl text-lg leading-relaxed text-zinc-600 sm:text-xl dark:text-zinc-400">
            {t.hero.subtitle}
          </p>
          <button
            type="button"
            onClick={scrollToDownload}
            className="mt-8 inline-flex items-center gap-2 rounded-xl bg-zinc-900 px-5 py-3 text-sm font-semibold text-white transition-all duration-300 hover:-translate-y-0.5 hover:bg-zinc-800 hover:shadow-xl hover:shadow-zinc-900/10 dark:bg-white dark:text-black dark:hover:bg-white/90 dark:hover:shadow-white/10"
          >
            {t.hero.btn} <ArrowDown size={16} />
          </button>
          <p className="mt-4 text-xs font-medium tracking-wide text-zinc-500">{t.hero.note}</p>
        </motion.div>

        <motion.div style={{ y: dashboardY }} className="relative mx-auto mt-5 max-w-5xl">
          <div className="absolute -inset-8 -z-10 rounded-[2.25rem] bg-blue-500/10 blur-3xl dark:bg-blue-500/15" />
          <div className="overflow-hidden rounded-2xl border border-zinc-200 bg-white shadow-[0_0_80px_rgba(59,130,246,0.12)] ring-1 ring-zinc-900/[0.03] dark:border-white/10 dark:bg-[#111114] dark:shadow-glow dark:ring-white/[0.03]">
            <div className="flex h-11 items-center justify-between border-b border-zinc-200 bg-zinc-50 px-4 dark:border-white/[0.08] dark:bg-[#151518]">
              <div className="flex items-center gap-1.5">
                <span className="h-2.5 w-2.5 rounded-full bg-red-400/70" />
                <span className="h-2.5 w-2.5 rounded-full bg-amber-300/70" />
                <span className="h-2.5 w-2.5 rounded-full bg-emerald-400/70" />
              </div>
              <div className="rounded-md border border-zinc-200 bg-zinc-100 px-12 py-1 text-[10px] text-zinc-500 dark:border-white/[0.06] dark:bg-black/20">{t.ui.dashboardUrl}</div>
              <MoreHorizontal size={17} className="text-zinc-400 dark:text-zinc-600" />
            </div>

            <div className="grid grid-cols-12 gap-3 p-3 sm:gap-4 sm:p-5">
              <aside className="col-span-3 hidden rounded-xl border border-zinc-200 bg-zinc-50 p-3 sm:block dark:border-white/[0.06] dark:bg-black/10">
                <div className="mb-6 flex items-center gap-2 text-xs font-semibold text-zinc-900 dark:text-zinc-100"><Sprout size={15} className="text-emerald-500 dark:text-emerald-300" /> {t.ui.field}</div>
                {sidebarItems.map((item, index) => (
                  <div key={item} className={`mb-1 rounded-lg px-2 py-2 text-[11px] ${index === 0 ? 'bg-zinc-200 text-zinc-950 dark:bg-white/[0.09] dark:text-white' : 'text-zinc-500'}`}>{item}</div>
                ))}
              ...
              <div className="col-span-12 space-y-3 sm:col-span-9 sm:space-y-4">
                <div className="flex items-center justify-between">
                  <div><p className="text-xs text-zinc-500">{t.ui.goodMorning}</p><h2 className="mt-1 text-sm font-semibold text-zinc-900 sm:text-base dark:text-zinc-100">{t.ui.fieldOverview}</h2></div>
                ...
                <div className="rounded-xl border border-zinc-200 bg-zinc-50 p-3 sm:p-4 dark:border-white/[0.07] dark:bg-white/[0.025]">
                  <div className="mb-4 flex items-center justify-between"><div><p className="text-xs font-medium text-zinc-800 dark:text-zinc-200">{t.ui.moistureTrend}</p><p className="mt-1 text-[10px] text-zinc-500">{t.ui.lastHours}</p></div><span className="text-[10px] text-emerald-600 dark:text-emerald-300">+4.8%</span></div>
                  <div className="relative h-24 overflow-hidden sm:h-28">
                    <div className="absolute inset-x-0 top-1/4 border-t border-dashed border-zinc-200 dark:border-white/[0.08]" /><div className="absolute inset-x-0 top-2/4 border-t border-dashed border-zinc-200 dark:border-white/[0.08]" /><div className="absolute inset-x-0 top-3/4 border-t border-dashed border-zinc-200 dark:border-white/[0.08]" />
